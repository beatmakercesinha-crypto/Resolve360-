...
interface ClientAreaProps {
  ...
  onOpenWarranty?: (bookingId?: string, serviceTitle?: string) => void;
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-slate-800">
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-sm max-w-2xl leading-relaxed">
              {isAuthenticated
                ? `Gerencie seus agendamentos técnicos no ${currentUser?.neighborhood || 'Pelotas'}, acompanhe o deslocamento do técnico em tempo real e consulte o Passaporte Digital da sua casa.`
                : 'Acompanhe seus serviços solicitados, rastreie técnicos a caminho e consulte a garantia blindada de 90 dias em Pelotas.'}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {onOpenPassport && (
              <button
                ...
                className="px-4 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-blue-300 font-bold text-xs rounded-xl transition-all flex items-center gap-2"
              >
                <QrCode className="w-4 h-4 text-blue-400" />
                <span>Passaporte do Imóvel</span>
              </button>
            )}
        ...
        <div className="mt-8 flex overflow-x-auto no-scrollbar gap-2 pb-2 border-b border-slate-800">
          ...
          >
            ...
            <span>Agendamentos & Rastreamento ({userBookings.length})</span>
          </button>
          ...
          >
            ...
            <span>Passaporte do Imóvel & QDC</span>
          </button>
        ...
        <div className="mt-8">
          ...
          {/* TAB 1: AGENDAMENTOS & LIVE TRACKING */}
          {activeTab === 'agendamentos' && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h3 className="text-xl font-bold text-white tracking-tight">
                    Visitas Técnicas & Rastreamento em Tempo Real
                  </h3>
                  <p className="text-xs text-slate-400">
                    Acompanhe o deslocamento do profissional com GPS em Pelotas, assine a garantia digital e avalie.
                  </p>
                </div>
                <button
                  onClick={() => onOpenSchedule()}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {userBookings.map((bk: Booking) => (
                    ...
                    >
                      ...
                      {/* Innovative Action Buttons (Live GPS, Digital Warranty) */}
                      <div className="pt-3 border-t border-slate-700/60 space-y-2">
                        
                        {/* Live Tracking Trigger */}
                        {bk.status === 'Confirmado' && onOpenTracking && (
                          <button
                            onClick={() => onOpenTracking(bk.id)}
                            className="w-full py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all shadow-md"
                          >
                            <Navigation className="w-3.5 h-3.5 animate-bounce" />
                            <span>Rastrear Técnico em Tempo Real</span>
                          </button>
                        )}

                        {/* Concluded service actions: Digital Warranty + Review */}
                        {bk.status === 'Concluído' ? (
                          <div className="space-y-2">
                            {onOpenWarranty && (
                              <button
                                onClick={() => onOpenWarranty(bk.id, bk.serviceTitle)}
                                className="w-full py-2 bg-slate-700 hover:bg-slate-600 text-emerald-300 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-colors border border-emerald-500/30"
                              >
                            ...
                            {bk.rating ? (
                              <div className="flex items-center justify-between p-2 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs">
                              ...
                              <button
                                ...
                                className="w-full py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all shadow-xs"
                              >
          ...
          {/* TAB 2: MINHA RESIDÊNCIA & PASSAPORTE DIGITAL */}
          {activeTab === 'casa' && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h3 className="text-xl font-bold text-white tracking-tight flex items-center gap-2">
                    <span>Passaporte Digital da Residência</span>
                    <span className="text-[10px] bg-blue-500/20 text-blue-300 border border-blue-400/30 px-2 py-0.5 rounded-full font-bold">
                      Inovação Pelotas
                    </span>
                  </h3>
                  <p className="text-xs text-slate-400">
                    Histórico predial da rede elétrica 220V, mapa de disjuntores e adesivo QR Code blindado.
                  </p>
                </div>
                {onOpenPassport && (
                  <button
                    onClick={onOpenPassport}
                    className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl transition-all shadow-md flex items-center gap-2 self-start"
                  >
                    <QrCode className="w-4 h-4" />
                    <span>Abrir Passaporte Completo & Imprimir QR Code</span>
                  </button>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-slate-800/80 border border-slate-700/80 rounded-3xl p-6 space-y-2">
                  ...
                  <div className="text-lg font-black text-white">
                    {currentUser?.savedProperties?.voltagem || '220V Bifásico'}
                  </div>
                  <p className="text-xs text-slate-400">
                    Padrão CEEE Equatorial Pelotas. 8 circuitos mapeados com proteção DPS e DR.
                  </p>
                </div>
                ...
                <div className="bg-slate-800/80 border border-slate-700/80 rounded-3xl p-6 space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Cobertura total sem zonas cegas na sala, quartos e pátio gourmet.
                  </p>
                </div>
                ...
                <div className="bg-slate-800/80 border border-slate-700/80 rounded-3xl p-6 space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Garantia estendida da mão de obra para todos os serviços com laudo técnico digital.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  },
                  {
                    ...
                    desc: 'Instalação de fechadura digital biométrica e câmera de entrada.',
                    local: 'Areal, Pelotas',
                    data: '20/07/2026',
                    status: 'Concluído',
                    rating: 5
